...
void SignalFunction::fourierTranseform(double inputSig[], int InputWidth){
	...
	for (int fre = 0; fre < m_DataArrSize; fre++) {		// m_FTOut에 저장되는 값은 특정 freq에서 실수&허수 성분 크기
		T_Real = T_Image = 0;
		for (int n = 0; n < m_DataArrSize; n++) {
	...
	half_switch_after_FT();
}

void SignalFunction::multiplication(double inputSig1[], double inputSig2[], int InputWidth) {
	m_DataArrSize = InputWidth;
	...
	for (int k = 0; k < m_DataArrSize; k++) {
		m_SigResult[k] += inputSig1[k] * inputSig2[k];
	}
}

void SignalFunction::half_switch_after_FT() {
	int half = m_DataArrSize / 2;
	for (int i = 0; i < half; i++) {
		double temp = m_SigResult[i];
		m_SigResult[i] = m_SigResult[half + i];
		m_SigResult[half + i] = temp;
	}
}