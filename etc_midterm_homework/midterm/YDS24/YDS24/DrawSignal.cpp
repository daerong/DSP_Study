...
void DrawSignal::setComplexSignal(char *type, _complex inputData[], int volume) {

	double *instance = new double[volume];

	if (type == "An" || type == "Cos" || type == "cos" || type == "x" || type == "X") {
		for (int i = 0; i < volume; i++) {
			instance[i] = inputData[i].x;
		}
	}
	else if (type == "Bn" || type == "Sin" || type == "sin" || type == "y" || type == "Y") {
		for (int i = 0; i < volume; i++) {
			instance[i] = inputData[i].y;
		}
	}
	else {
		for (int i = 0; i < volume; i++) {
			instance[i] = sqrt(pow(inputData[i].x, 2.0) + pow(inputData[i].y, 2.0));
		}
	}
	...
	memcpy(m_SigData, instance, sizeof(double)*m_DataArrSize);
